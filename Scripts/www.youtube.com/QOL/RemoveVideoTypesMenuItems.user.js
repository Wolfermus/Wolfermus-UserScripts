 ...
 * @returns { boolean }
 */
async function ShouldDisable(json, json2) {
    const currentWebsitesObject = GetCurrentWebsitesObject(json);
...
function GenerateValueChangedCallback(QOLRemoveVideoTypesPlaylistMenuItem, QOLRemoveVideoTypesPageTypeGroupMenuItem, QOLRemoveVideoTypesTogglePageMenuItem, QOLRemoveVideoTypesGroupMenuItem, QOLRemoveVideoTypesMenuItem) {
    return async (key, oldValue, newValue, remote) => {
        ...
        if (shouldSave) await SetValue("YoutubeQOL", JSON.stringify(QOLSettings));
    }
}
...
async function EntryRun(baseURL, baseScriptURL, baseWebsiteScriptURL, branch) {
    ...
    QOLRemoveVideoTypesPlaylistMenuItem.ToggledEventAddCallback(async (toggled) => {
        ...
        if (shouldSave) await SetValue("YoutubeQOL", JSON.stringify(QOLSettingsInner));
    });
    const keysArray = GetCurrentWebsitesEnabled();
    if (keysArray.length === 1) {
